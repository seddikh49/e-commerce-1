...
const ProductItems = ({latestProducts}) => {
  ...
  return (
    <div>
        <div className='grid lg:grid-cols-3 md:grid-cols-2  sm:grid-cols-2 xl:grid-cols-4 xm:grid-cols-1 gap-y-6 gap-4'>
        { latestProducts.map((products)=> {
             return(
              <Link key={products._id} to={`product/${products._id}`} className=''>
                <img className='sm:w-full xm:w-full hover:scale-105 transition-all duration-300' src={products.image[0]} alt="" />
